...
export type ArmyInfo = ComponentValue<ClientComponents['Army']['schema']> & {
  ...
  protectee:
    | ComponentValue<ClientComponents['Protectee']['schema']>
    | undefined;
  movable: ComponentValue<ClientComponents['Movable']['schema']> | undefined;
  totalCapacity: bigint;
  weight: bigint;
  arrivalTime:
    | ComponentValue<ClientComponents['ArrivalTime']['schema']>
    | undefined;
  ...
  homePosition:
    | ComponentValue<ClientComponents['Position']['schema']>
    | undefined;
};
...
const formatArmies = (
  ...
  return armies
    .map((armyEntityId) => {
      ...
      const armyCapacityConfigEntityId = getEntityIdFromKeys([
        BigInt(CapacityConfigCategory.Army),
      ]);
      const capacity = getComponentValue(
        CapacityConfig,
        armyCapacityConfigEntityId
      );
      const totalCapacity = capacity
        ? getArmyTotalCapacity(army, capacity)
        : 0n;

      const weightComponentValue = getComponentValue(Weight, armyEntityId);
      const weight = weightComponentValue
        ? weightComponentValue.value /
          BigInt(EternumGlobalConfig.resources.resourcePrecision)
        : 0n;
      ...
      const homePosition =
        ...
        getComponentValue(
          Position,
          getEntityIdFromKeys([BigInt(realm.entity_id)])
        );
...
export const useArmiesByEntityOwner = ({
...
export const useArmyByArmyEntityId = (entityId: ID): ArmyInfo | undefined => {
  ...
  const armiesEntityIds = useEntityQuery([
    HasValue(Army, { entity_id: entityId }),
  ]);
  return formatArmies(
    Array.from(armiesEntityIds),
    account.address,
...
}) => {
  {
    ...
    const ownArmies = useMemo(() => {
      return formatArmies(
        ...
        playerStructures.some(
          (structure) =>
            structure.entity_id === army.entityOwner.entity_owner_id
        )
      );
