...
export type PlayerStructure = ComponentValue<
...
export const useEntities = () => {
  ...
  const address = ContractAddress('0x0');
  ...
  const filterPlayerRealms = useMemo(() => {
    return allRealms.filter((id) => {
      ...
      return (
        owner && ContractAddress(owner.address) === ContractAddress(address)
      );
    });
  }, [allRealms, address]);
  ...
  // Get all structures
  const allStructures = useEntityQuery([
    Has(Structure),
    Has(Position),
    Has(Owner),
  ]);
  ...
  const playerStructures = useMemo(() => {
    return filterPlayerStructures
      .map((id) => {
        ...
        const name = realm
          ? getRealmName(realm)
          : structureName || structure.category || '';
      ...
      .filter(
        (structure): structure is PlayerStructure => structure !== undefined
      )
      .sort((a, b) => {
  ...
  const getPlayerStructures = (
    filterFn?: (structure: PlayerStructure) => boolean
  ) => {
    return useMemo(() => {
      const structures = filterFn
        ? playerStructures.filter(filterFn)
        : playerStructures;
...
export const useEntitiesUtils = () => {
  ...
  const getEntityInfo = (entityId: ID) => {
    ...
    const arrivalTime = getComponentValue(
      ArrivalTime,
      getEntityIdFromKeys([entityIdBigInt])
    );
    ...
    const entityCapacityCategory = getComponentValue(
      CapacityCategory,
      getEntityIdFromKeys([entityIdBigInt])
    )?.category as unknown as string;
    const capacityCategoryId =
      CAPACITY_CONFIG_CATEGORY_STRING_MAP[entityCapacityCategory] || 0n;
    const capacity = getComponentValue(
      CapacityConfig,
      getEntityIdFromKeys([BigInt(capacityCategoryId)])
    );
    ...
    return {
      ...
      owner: owner?.address,
      isMine:
        ContractAddress(owner?.address || 0n) ===
        ContractAddress(0n /*TBB account.address*/),
  ...
  const getEntityName = (entityId: ID, abbreviate: boolean = false) => {
    const entityName = getComponentValue(
      EntityName,
  ...
  const getAddressName = (address: ContractAddress) => {
    const addressName = getComponentValue(
      AddressName,
      getEntityIdFromKeys([BigInt(address)])
    );
  ...
  const getPlayerAddressFromEntity = (
    entityId: ID
