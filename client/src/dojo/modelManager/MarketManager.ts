...
export class MarketManager {
  ...
  public getPlayerLiquidity() {
    return getComponentValue(
      ...
      getEntityIdFromKeys([
        BigInt(this.bankEntityId),
        this.player,
        BigInt(this.resourceId),
      ])
    );
  }
  ...
  public quoteResource = (lordsAmount: number) => {
    ...
    let resourceOptimal =
      (reserveResourceAmount * lordsAmount) / reserveLordsAmount;
  ...
  public quoteLords = (resourceAmount: number) => {
    ...
    let lordsOptimal =
      (reserveLordsAmount * resourceAmount) / reserveResourceAmount;
  ...
  public getOutputAmount(
    ...
    const inputAmountWithFee =
      BigInt(inputAmount) * BigInt(feeRateDenom - feeRateNum);
    ...
    const denominator =
      BigInt(inputReserve) * BigInt(feeRateDenom) + inputAmountWithFee;
  ...
  public calculateResourceOutputForLordsInput = (
    lordsAmount: number,
    feeRateNum: number
  ) => {
    const market = this.getMarket();
    ...
    let outputAmount = this.getOutputAmount(
      lordsAmount,
      market.lords_amount,
  ...
  public calculateLordsInputForResourceOutput = (
    ...
    // Adjust for fees
    const inputAmountWithFee =
      (inputAmount * BigInt(feeRateDenom)) / BigInt(feeRateDenom - feeRateNum);

    return Number(inputAmountWithFee);
  };

  public calculateLordsOutputForResourceInput = (
    resourceAmount: number,
    feeRateNum: number
  ) => {
    const market = this.getMarket();
    ...
    let inputPrice = this.getInputPrice(
      resourceAmount,
      market.resource_amount,
      market.lords_amount,
      feeRateNum
  ...
  public calculateResourceInputForLordsOutput = (
    lordsAmount: number,
    feeRateNum: number
  ) => {
    const market = this.getMarket();
  ...
  public slippage = (inputAmount: number, isBuyResource: boolean) => {
    ...
    const marketPriceAmount = isBuyResource
      ? inputAmount / marketPrice
      : inputAmount * marketPrice;

    // Calculate the slippage percentage
    const slippagePercentage =
      ((marketPriceAmount - outputAmount) / marketPriceAmount) * 100;
  ...
  public getMyLP() {
    ...
    let lords_amount = Number(
      (my_shares * BigInt(reserveLordsAmount)) / BigInt(total_shares)
    );
    let resource_amount = Number(
      (my_shares * BigInt(reserveResourceAmount)) / BigInt(total_shares)
    );
  ...
  public getMyFees(
    initial_lords_amount: number,
    initial_resource_amount: number
  ) {
    const [current_lords_amount, current_resource_amount] = this.getMyLP();
    return [
      current_lords_amount - initial_lords_amount,
      current_resource_amount - initial_resource_amount,
    ];
  }
  ...
  public getReserves() {
    const market = this.getMarket();
    return [
      Number(market?.lords_amount || 0n),
      Number(market?.resource_amount || 0n),
    ];
  }

  public getLatestLiquidityEvent(playerStructureIds: ID[]) {
    let mostRecentEvent: ComponentValue<
      ClientComponents['events']['LiquidityEvent']['schema']
    > | null = null;
    ...
    playerStructureIds.forEach((structureId) => {
      ...
      liquidityEvents.forEach((event) => {
        const eventInfo = getComponentValue(
          this.setup.components.events.LiquidityEvent,
          event
        );
        if (
          eventInfo &&
          (!mostRecentEvent || eventInfo.timestamp > mostRecentEvent.timestamp)
        ) {
    ...
    return mostRecentEvent as ComponentValue<
      ClientComponents['events']['LiquidityEvent']['schema']
    > | null;
  }
}
