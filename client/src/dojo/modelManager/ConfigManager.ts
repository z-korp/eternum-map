...
export class ClientConfigManager {
  ...
  private initializeResourceInputs() {
    ...
    // }
    this.resourceInputs = Object.entries(
      EternumGlobalConfig.resources.resourceInputs
    ).reduce((acc, [key, inputs]) => {
      acc[Number(key)] = inputs.map(
        (input: { resource: number; amount: number }) => ({
  ...
  private initializeRealmUpgradeCosts() {
    ...
    this.realmUpgradeCosts = Object.fromEntries(
      Object.entries(EternumGlobalConfig.realmUpgradeCosts).map(
        ([key, costs]) => [
          key,
          costs.map((cost: any) => ({
            ...cost,
            amount: cost.amount * this.getResourceMultiplier(),
          })),
        ]
      )
    );
  ...
  private initializeResourceBuildingCosts() {
    ...
    this.resourceBuildingCosts = Object.fromEntries(
      Object.entries(EternumGlobalConfig.resources.resourceBuildingCosts).map(
        ([key, costs]) => [
          key,
          costs.map((cost: any) => ({
            ...cost,
  ...
  private initializeBuildingCosts() {
    ...
    this.buildingCosts = Object.fromEntries(
      Object.entries(EternumGlobalConfig.buildings.buildingCosts).map(
        ([key, costs]) => [
          key,
          costs.map((cost: any) => ({
            ...cost,
  ...
  getExploreReward() {
    return this.getValueOrDefault(() => {
      const exploreConfig = getComponentValue(
        this.components.MapConfig,
        getEntityIdFromKeys([WORLD_CONFIG_ID])
      );

      return divideByPrecision(
        Number(exploreConfig?.reward_resource_amount ?? 0)
      );
    }, 0);
  }
      ...
      () => {
        ...
        return {
          ...
          armyExtraPerMilitaryBuilding:
            troopConfig?.army_extra_per_building ?? 0,
  ...
  getResourceBridgeFeeSplitConfig() {
    return this.getValueOrDefault(
      () => {
        ...
        return {
          config_id: Number(
            resourceBridgeFeeSplitConfig?.config_id ?? WORLD_CONFIG_ID
          ),
          ...
          max_bank_fee_dpt_percent: Number(
            resourceBridgeFeeSplitConfig?.max_bank_fee_dpt_percent ?? 0
          ),
          max_bank_fee_wtdr_percent: Number(
            resourceBridgeFeeSplitConfig?.max_bank_fee_wtdr_percent ?? 0
          ),
        };
      },
      {
  ...
  getAdminBankOwnerFee() {
    const adminBank = getComponentValue(
      this.components.Bank,
      getEntityIdFromKeys([ADMIN_BANK_ENTITY_ID])
    );
  ...
  getCapacityConfig(category: CapacityConfigCategory) {
    return this.getValueOrDefault(() => {
      const capacityConfig = getComponentValue(
        ...
        getEntityIdFromKeys([BigInt(category)])
      );
  ...
  getHyperstructureTotalContributableAmount(hyperstructureId: number) {
    ...
    return requiredAmounts.reduce(
      (total, { amount, resource }) =>
        total + amount * configManager.getResourceRarity(resource),
      0
    );
  }
  ...
  getBasePopulationCapacity(): number {
    return this.getValueOrDefault(() => {
      return (
        getComponentValue(
          this.components.PopulationConfig,
          getEntityIdFromKeys([POPULATION_CONFIG_ID])
        )?.base_population ?? 0
      );
    }, 0);
  }
      ...
      () => {
        ...
        return {
          exploreWheatBurnAmount:
            Number(travelFoodCostConfig?.explore_wheat_burn_amount) ?? 0,
          exploreFishBurnAmount:
            Number(travelFoodCostConfig?.explore_fish_burn_amount) ?? 0,
          travelWheatBurnAmount:
            Number(travelFoodCostConfig?.travel_wheat_burn_amount) ?? 0,
          travelFishBurnAmount:
            Number(travelFoodCostConfig?.travel_fish_burn_amount) ?? 0,
        };
      },
      {
  ...
  getResourceBuildingProduced(buildingType: BuildingType) {
    return (
      EternumGlobalConfig.buildings.buildingResourceProduced[buildingType] ?? 0
    );
  }
  ...
  getSeasonBridgeConfig() {
    return this.getValueOrDefault(
      () => {
        ...
        return {
          closeAfterEndSeconds:
            seasonBridgeConfig?.close_after_end_seconds ?? 0n,
        };
      },
      {
  ...
  getSeasonConfig() {
    return this.getValueOrDefault(
      () => {
        const season = getComponentValue(
          this.components.Season,
          getEntityIdFromKeys([WORLD_CONFIG_ID])
        );
        return {
  ...
  getWeightLessResources() {
    return this.getValueOrDefault(() => {
      ...
      for (const resourceId of Object.values(ResourcesIds).filter(
        Number.isInteger
      )) {
